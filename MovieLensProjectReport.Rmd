...
This section covers the exploratory data analysis carried out and associated findings.


### 2.1 Data sparseness
An initial plot of 100 randomly selected users, and the coverage of films they've reviewed suggests a sparse matrix, with each user unsurprisingly having only reviewed a small minority of the overall movies available.
...
### 2.2 Highest/Lowest Rated Movies and Most/Least Frequently Reviewed Movies


Would expect the weakest algo performance for the movies with the least reviews

### 2.3 Highest/Lowest Rated Genres and distribution of vol of reviews for each genre (chart)

Would expect the weakest algo performance for the genres with the least reviews


### 2.4 Movies by Year; distribution of reviews

### 2.5 Timestamps of Reviews: distribution across dates + distribution of reviews during the day

### 2.6 Any correlation of year of production or time of day / date of review with ratings


### 2.7 NB: Are the genres for each film consistent? Assume so, assume they've come from movies dataset. But check.
If so, see if any correlation between number of different genres assigned to a film and the rating. 
More genres = lower rating as a hypothesis?
